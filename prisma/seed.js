...
const UNIVERSAL_STARTER_CODE = `import java.util.*;
...
async function main() {
  console.log(`Seeding ${topics.length} topics and ${questions.length} curated questions...`);

  // Ensure default learner exists
  await prisma.user.upsert({
    where: { email: "learner@javatrainer.dev" },
    update: {},
    create: {
  ...
  // Clear obsolete question records that are not in Top 50 list
  const validIds = new Set(questions.map((q) => q.id));
  const existingQs = await prisma.question.findMany({ select: { id: true } });
  for (const eq of existingQs) {
    if (!validIds.has(eq.id)) {
      await prisma.submission.deleteMany({ where: { questionId: eq.id } });
      await prisma.question.delete({ where: { id: eq.id } });
    }
  }
  ...
  console.log(`Successfully seeded ${questions.length} curated Top 50 questions!`);
}
