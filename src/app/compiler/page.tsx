...
const RAPID_STARTER_CODE = `// ⚡ Rapid Mode: Write raw Java statements directly!
// No imports, classes, or main methods needed.
...
const FULL_CLASS_STARTER_CODE = `import java.util.*;
...
export default function OnlineCompilerPage() {
  const [mode, setMode] = useState<"rapid" | "full">("rapid");
  ...
  // Switch between Rapid Mode and Full Class Mode
  const handleModeSwitch = (newMode: "rapid" | "full") => {
    setMode(newMode);
    if (newMode === "rapid") {
      setCode(RAPID_STARTER_CODE);
    } else {
      setCode(FULL_CLASS_STARTER_CODE);
    }
    setError(null);
    setOutput("");
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 py-2.5 border-b border-slate-200 bg-white shrink-0 shadow-2xs z-10">
        {/* Left: Mode Toggle & Title */}
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <h1 className="font-extrabold text-sm text-slate-900 hidden sm:block">
              Rapid Java Compiler
            </h1>
          </div>
          ...
          <div className="flex items-center p-0.5 rounded-full bg-slate-100 border border-slate-200">
            <button
              onClick={() => handleModeSwitch("rapid")}
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold transition-all ${
                mode === "rapid"
                  ? "bg-amber-500 text-white shadow-xs"
                  : "text-slate-600 hover:text-slate-900"
              }`}
              title="Write raw statements directly without classes or main methods"
            >
            ...
            <button
              ...
              title="Traditional full Java class format (public class Main)"
            >
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            onClick={() =>
              setCode(mode === "rapid" ? RAPID_STARTER_CODE : FULL_CLASS_STARTER_CODE)
            }
        ...
        <div className="px-6 py-1.5 bg-slate-50/80 border-b border-slate-100 text-[11px] font-mono text-slate-500 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-700">
              {mode === "rapid" ? "⚡ Rapid Snippet (No Boilerplate)" : "Main.java"}
            </span>
